...
export class GameScene extends Phaser.Scene {
  ...
  update(): void {
    ...
    this.player.update();
    this.coin.update();
    if(this.player.x <=0 || this.player.x >= 768 || this.player.y <=0 || this.player.y >= 576){
      this.coinsCollectedText.setText("GAME OVER " + this.collectedCoins + "");
      // this.coinsCollectedText.setPosition(window.document.getElementById("game").offsetWidth/2,window.document.getElementById("game").offsetHeight/2);
